...
class NAS201SearchCell(nn.Module):
  ...
  def __init__(self, C_in, C_out, stride, max_nodes, op_names_list, affine=False, track_running_stats=True):
    ...
    for i in range(1, max_nodes):
      for j in range(i):
        self.num_edges += 1
    if type(op_names_list[0]) is list:
      op_names_list = op_names_list
    else:
      op_names_list = [op_names_list] * self.num_edges
    self.op_names_list  = deepcopy(op_names_list)
    index = 0
    for i in range(1, max_nodes):
      for j in range(i):
        node_str = '{:}<-{:}'.format(i, j)
        if j == 0:
          xlists = [OPS[op_name](C_in, C_out, stride, affine, track_running_stats) for op_name in op_names_list[index]]
        else:
          xlists = [OPS[op_name](C_in, C_out, 1, affine, track_running_stats) for op_name in op_names_list[index]]
        index += 1
  ...
  def forward_dynamic(self, inputs, structure):
    ...
    for i in range(1, self.max_nodes):
      ...
      for op_name, j in cur_op_node:
        node_str = '{:}<-{:}'.format(i, j)
        # print(node_str)
        op_index = self.op_names_list[index].index( op_name )
