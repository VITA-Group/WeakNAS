...
class MobileNetArchEncoder:
	...
	def arch2feature(self, arch_dict):
		ks, e, d, r = arch_dict['ks'], arch_dict['e'], arch_dict['d'], arch_dict['r']
	...
	def random_sample_arch(self):
		return {
			...
			'r': random.choice(self.image_size_list)
		}

	def mutate_resolution(self, arch_dict, mutate_prob):
		if random.random() < mutate_prob:
			arch_dict['r'] = random.choice(self.image_size_list)
		return arch_dict
...
class ResNetArchEncoder:
	...
	def arch2feature(self, arch_dict):
		d, e, w, r = arch_dict['d'], arch_dict['e'], arch_dict['w'], arch_dict['r']
