...
class RunManager:
    ...
    def __init__(self, path, net, run_config, init=True, measure_latency=None, no_gpu=False):
        ...
        # model info
        net_info = get_net_info(self.net, self.run_config.data_provider.data_shape, measure_latency, False)
        with open('%s/net_info.txt' % self.path, 'w') as fout:
            ...
            try:
                ...
                fout.write('%s\n' % self.run_config.data_provider.train.dataset.dataset.transform)
                fout.write('%s\n' % self.run_config.data_provider.test.dataset.transform)
            fout.write('%s\n' % self.network)
    ...
    def save_config(self, extra_run_config=None, extra_net_config=None):
        ...
        try:
            net_save_path = os.path.join(self.path, 'model.config')
            ...
            print('%s do not support model config' % type(self.network))
    ...
    def get_grads(self):
        ...
        for (name, value) in self.net.named_parameters():
            ...
            if value.grad is not None:
                grads = value.grad.cpu().detach().numpy()
                # len(grads.shape)
                self.grad_dict[name] = grads
                # i = int(name.split('.')[1])

    def cal_grads_mean(self):
        for key, value in self.grad_dict.items():
            self.grad_dict[key] = np.mean(value)
