		...
		for j, (width, n_block, s, act_func, use_se) in enumerate(zip(width_list[2:], n_block_list[1:],
			...
			for i in range(n_block):
				if i == 0:
					...
					stride = 1
				print(f'Index {_block_index}: {j} {i} {4*j+i}') #, {feature_dim} {output_channel}
...
if __name__ == "__main__":
	...
	net_setting = {"ks": [5, 3, 3, 3,
				   ...
				   "e":  [3, 3, 4, 3,
						  ...
						  6, 6, 6, 3],
				   "d":  [4, 3, 3, 3, 4]}
	net = OFAMobileNetV3(dropout_rate=0, width_mult=1.2, ks_list=net_setting["ks"], expand_ratio_list=net_setting["e"], depth_list=net_setting["d"],)
