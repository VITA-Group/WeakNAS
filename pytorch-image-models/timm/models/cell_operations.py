...
OPS = {
  ...
  'none_bn'      : lambda C_in, C_out, stride, affine, track_running_stats: ZeroBN(C_in, C_out, stride),
  ...
  'avg_pool_3x3_bn': lambda C_in, C_out, stride, affine, track_running_stats: POOLINGBN(C_in, C_out, stride, 'avg', affine, track_running_stats),
  ...
  'max_pool_3x3_bn': lambda C_in, C_out, stride, affine, track_running_stats: POOLINGBN(C_in, C_out, stride, 'max', affine, track_running_stats),
  ...
  'nor_conv_5x5':  lambda C_in, C_out, stride, affine, track_running_stats: ReLUConvBN(C_in, C_out, (5,5), (stride,stride), (2,2), (1,1), affine, track_running_stats),
  ...
  'nor_conv_3x3_bn': lambda C_in, C_out, stride, affine, track_running_stats: ReLUConvBN(C_in, C_out, (3, 3),(stride, stride), (1, 1), (1, 1), affine, track_running_stats),
  ...
  'nor_conv_1x1_bn': lambda C_in, C_out, stride, affine, track_running_stats: ReLUConvBN(C_in, C_out, (1, 1), (stride, stride), (0, 0), (1, 1), affine, track_running_stats),
  ...
  'skip_connect_bn' : lambda C_in, C_out, stride, affine, track_running_stats: IdentityBN(C_in) if stride == 1 and C_in == C_out else FactorizedReduce(C_in, C_out, stride, affine, track_running_stats),
}
...
class ReLUConvBNBN(nn.Module):

  def __init__(self, C_in, C_out, kernel_size, stride, padding, dilation, affine, track_running_stats=True):
    super(ReLUConvBNBN, self).__init__()
    self.op = nn.Sequential(
      nn.ReLU(inplace=False),
      nn.Conv2d(C_in, C_out, kernel_size, stride=stride, padding=padding, dilation=dilation, bias=False),
      nn.BatchNorm2d(C_out, affine=affine, track_running_stats=track_running_stats)
...
class POOLINGBN(nn.Module):

  def __init__(self, C_in, C_out, stride, mode, affine=True, track_running_stats=True):
    super(POOLINGBN, self).__init__()
    if C_in == C_out:
      self.preprocess = None
    else:
      self.preprocess = ReLUConvBN(C_in, C_out, 1, 1, 0, 1, affine, track_running_stats)
    ...
    self.bn = nn.BatchNorm2d(C_in, affine=affine, track_running_stats=track_running_stats)
...
class IdentityBN(nn.Module):
  ...
  def __init__(self, C_in, affine=True, track_running_stats=True):
