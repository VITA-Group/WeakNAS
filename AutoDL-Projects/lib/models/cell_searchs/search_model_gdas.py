...
class MobileNetGDAS(nn.Module):
  ...
  def forward(self, x):
    ...
    alphas_full = [hardwts[i] for i, s in enumerate(self.stages_len) for _ in range(s)]
    x = self.first_conv(x)
    # inter_nodes.append(sum(layer(nodes[j]) * w for layer, w in zip(self.edges[node_str], weights)))
    for i, (features, alpha) in enumerate(zip(self.features, alphas_full)):
        x = sum([layer(x) * w for layer, w in zip(features, alpha) if w])
        # x = features[torch.argmax(alpha)](x)
    x = self.conv_last(x)
    x = self.globalpool(x)
    x = self.last(x)
    x = x.view(x.size(0), -1)
