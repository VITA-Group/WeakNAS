...
from .tinyimagenet import ImageFolderTensor
...
def get_datasets(name, root, cutout, crop_size=64):
  ...
  if name == 'cifar10' or name == 'cifar100':
    ...
    lists = [transforms.ToPILImage(), transforms.RandomResizedCrop(64), transforms.RandomHorizontalFlip(), transforms.ToTensor(), transforms.Normalize(mean, std)]
    if cutout > 0 : lists += [CUTOUT(cutout)]
    train_transform = transforms.Compose(lists)
    test_transform  = transforms.Compose([transforms.ToPILImage(), transforms.CenterCrop(64), transforms.ToTensor(), transforms.Normalize(mean, std)])
    xshape = (1, 3, 64, 64)
  elif name.startswith('imagenet-1k'):
  ...
  if name == 'cifar10':
    ...
    train_data = ImageFolderTensor(os.path.join(root, 'TinyImageNet/train.tensor'), train_transform)
    test_data = ImageFolderTensor(os.path.join(root, 'TinyImageNet/val.tensor'), test_transform)
