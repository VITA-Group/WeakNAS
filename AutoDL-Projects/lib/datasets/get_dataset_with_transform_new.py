...
def get_datasets(name, root, cutout, crop_size=64):
  ...
  if name == 'cifar10' or name == 'cifar100':
    ...
    lists = [transforms.RandomResizedCrop(crop_size), transforms.RandomHorizontalFlip(), transforms.ToTensor(), transforms.Normalize(mean, std)]
    if cutout > 0 : lists += [CUTOUT(cutout)]
    train_transform = transforms.Compose(lists)
    test_transform  = transforms.Compose([transforms.CenterCrop(crop_size), transforms.ToTensor(), transforms.Normalize(mean, std)])
    xshape = (1, 3, 64, 64)
  elif name.startswith('imagenet-1k'):
...
def get_nas_search_loaders_new(train_data, valid_data, dataset, config_root, batch_size, workers, train=False):
  ...
  if dataset == 'cifar10':
    ...
    test_loader  = torch.utils.data.DataLoader(valid_data , batch_size=test_batch, num_workers=workers, pin_memory=True)
    if train:
    ...
    test_loader  = torch.utils.data.DataLoader(valid_data , batch_size=test_batch, sampler=torch.utils.data.sampler.SubsetRandomSampler(cifar100_test_split.xtest), num_workers=workers, pin_memory=True)
    if train:
      return search_loader, train_loader, valid_loader, test_loader
  elif dataset == 'TinyImageNet':
    search_train_data = train_data
    search_valid_data = deepcopy(valid_data) ; search_valid_data.transform = train_data.transform
    search_data   = SearchDataset(dataset, [search_train_data,search_valid_data], train_split=list(range(len(search_train_data))), valid_split=list(range(0, len(valid_data), 2)))
    search_loader = torch.utils.data.DataLoader(search_data, batch_size=batch, shuffle=True , num_workers=workers, pin_memory=True)
    if train:
      train_loader  = torch.utils.data.DataLoader(train_data , batch_size=batch, shuffle=True , num_workers=workers, pin_memory=True)
    valid_loader  = torch.utils.data.DataLoader(valid_data , batch_size=test_batch, sampler=torch.utils.data.sampler.SubsetRandomSampler(list(range(0, len(valid_data), 2))), num_workers=workers, pin_memory=True)
    test_loader  = torch.utils.data.DataLoader(valid_data , batch_size=test_batch, sampler=torch.utils.data.sampler.SubsetRandomSampler(list(range(1, len(valid_data), 2))), num_workers=workers, pin_memory=True)
    if train:
      return search_loader, train_loader, valid_loader, test_loader
  ...
  return search_loader, valid_loader, test_loader
